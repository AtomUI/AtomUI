...
舞台行为（2026-09-25 第八轮校准后终态，用户确认的放大语义）：**放大 = 给内容更大的展示空间**。
`ShowCaseZoomStagePanel` 以**舞台宽度**测量内容（高度无限），按内容本性二分：
- **铺满型内容**（期望宽度跟随所给宽度的控件：24 栅格 Col、表格等）按舞台宽度**重新布局并铺满**；
- **固定尺寸内容**（按钮组、单控件等）按期望尺寸**水平垂直居中**（默认）。

不做任何拉伸或 RenderTransform 缩放（铺满是内容自适应重排，不是外部变换）。历史机制已退役：
`StageMeasureWidth` 捕获宽度保真（第三轮引入，因固定示例留白与用户最终语义确认移除）、缩放机制
（两进两出）。教训链：v1 无限测量废对齐 → v2 无上限缩放失真 → v3 舞台宽测量（当时因页面级样式
丢失等未查明的独立缺陷被误判否定）→ v3.1 冻结宽度空旷 → v5 有限缩放违和 → v6 零缩放 →
**v8 终态：舞台宽测量 + 铺满/居中二分**。
